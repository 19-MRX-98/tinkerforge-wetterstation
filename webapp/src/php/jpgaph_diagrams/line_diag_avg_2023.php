<?php
    ...
    require_once ('src/jpgraph-4.4.2/src/jpgraph_ttf.inc.php');
        ...
        $get_avg_per_month = "SELECT MONTH(datetime) AS month, AVG(temperatur)/10 AS avg_temp FROM wetterdaten2023 GROUP BY MONTH(datetime)";
        $result_line1 = $conn->query($get_avg_per_month);

        // Datenbankabfrage für Linie 2
        $get_avg_long_term = "SELECT January, February, March, April, May, June, July, August, September, October, November, December FROM jahresmittel_1991_2020 WHERE selected = '1'";
        ...
        $graph->Stroke("src/php/jpgaph_diagrams/jpg/avg_chart_2023.jpg");
?>