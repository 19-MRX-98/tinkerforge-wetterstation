<?php
    ...
    $stmt = $db->prepare("SELECT MONTHNAME(datetime) AS monat, AVG(temperatur)/10 AS monatsmitteltemperatur FROM wetterdaten2022 GROUP BY MONTH(datetime);");
    $stmt->execute();
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_array($result)) {
            ...
            if (mysqli_num_rows($result2) > 0) {
                ...
                // 4. Füge die Monatsmitteltemperatur und die Abweichung in die abweichungen_2022 Tabelle ein
                $insert_stmt = $db->prepare("INSERT INTO abweichungen_2022 (monat, monatsmitteltemperatur, abweichung) VALUES (?, ?, ?)");
